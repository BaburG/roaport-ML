...
def callback(ch, method, properties, body):
    ...
    try:
        ...
        message_type = message.get('type').lower() # Get the new 'type' field
        report_id = message.get('report_id')  # Extract report_id for verification
        
        logger.info(f"Received message [{image_id}]: Type='{message_type}', URL={image_url}, Report ID={report_id}")
        ...
        if message_type == "pothole":
            ...
            if image_path:
                ...
                if pothole_exists and report_id:
                    send_verification_request(report_id)
            else:
            ...
            if image_path:
                ...
                if image_b64:
                    ...
                    if ai_response.strip().upper() == "YES" and report_id:
                        send_verification_request(report_id)
                else:
...
def send_verification_request(report_id):
    """Send verification request to the API when damage is detected"""
    try:
        verification_url = "https://roaport.com/api/verify"
        payload = {"reportId": report_id}
        headers = {"Content-Type": "application/json"}
        
        logger.info(f"Sending verification request for report_id: {report_id}")
        response = requests.post(verification_url, json=payload, headers=headers, timeout=30)
        response.raise_for_status()
        
        logger.info(f"Verification request successful for report_id: {report_id} - Status: {response.status_code}")
        return True
        
    except requests.exceptions.RequestException as e:
        logger.error(f"Failed to send verification request for report_id: {report_id} - Error: {e}")
        return False
    except Exception as e:
        logger.error(f"Unexpected error sending verification request for report_id: {report_id} - Error: {e}")
        return False
